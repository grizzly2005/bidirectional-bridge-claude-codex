...
function recoveryAdapter(clock: ManualClock, behavior: AdapterBehavior = {}) {
  ...
  const adapter: AgentAdapter = {
    ...
    async invoke(invocation, context) {
      ...
      const verification = complete(invocation, clock.now()).verification_results[0]!;
...
function createStranded(
  ...
  const parent = cp.tasks.create({
    spec: { ...spec("parent/**"), preferred_agent: "claude" },
    created_by: "claude",
    run_id: RUN_ID,
  });
  ...
  cp.tasks.transition({ task_id: child.task_id, agent: "codex", to: TaskState.WORKING });
  cp.attempts.start(child.task_id, 0, "codex");
  const handle = options.handle === undefined ? HANDLE : options.handle;
  if (handle) cp.attempts.saveHandle(child.task_id, 0, "codex", handle);
  if (!options.keepLease) cp.leases.release(lease.lease_id, "codex");
  cp.tasks.block(child.task_id, "codex", "runtime interrupted");
